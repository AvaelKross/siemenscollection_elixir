defmodule PhonesCollectionWeb.PhoneEdition do
  ...
  schema "phone_editions" do
    ...
    belongs_to :phone, PhonesCollectionWeb.Phone, foreign_key: :phone_id
  ...
  def owned_amount(edition, user) do
    case {edition, user} do
      {_, nil} -> 0
      {_, _} -> Repo.all(from i in PhonesCollectionWeb.Item, where: i.user_id == ^user.id, where: i.phone_edition_id == ^edition.id, select: count(i.id)) |> List.first
    end
  end
