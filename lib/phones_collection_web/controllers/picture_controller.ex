defmodule PhonesCollectionWeb.PictureController do
  ...
  def create(conn, %{"images" => images}) do
    ...
    # single-thread version (same speed???)
  ...
  def rotate(conn, %{"id" => id, "direction" => direction}) do
    ...
    {:ok, rotated_image} = Arc.Transformations.Convert.apply(:convert,
                                                             image,
                                                             transformation)
  ...
  def set_cover(conn, params) do
    ...
    changeset = if edition_id != nil do
      Repo.get!(PhonesCollectionWeb.PhoneEdition, edition_id)
      |> PhonesCollectionWeb.PhoneEdition.changeset(%{cover_id: id})
    else
      Repo.get!(PhonesCollectionWeb.Item, item_id)
      |> PhonesCollectionWeb.Item.changeset(%{cover_id: id})
    end
  ...
  defp redirect_path(conn, _) do
    path = if conn.params["edition_id"] != nil do
             Routes.catalog_phone_edition_path(conn, :edit, conn.params["brand_id"],
                                                     conn.params["phone_id"],
                                                     conn.params["edition_id"])
           else
             Routes.collections_item_path(conn, :edit, conn.params["user_id"],
                                                conn.params["item_id"])
           end
  ...
  defp check_rights(conn, _) do
    if conn.params["edition_id"] != nil do
      if PhonesCollectionWeb.Plugs.CheckAdminRights.can_edit(conn), do: conn, else: redirect_no_rights(conn)
    else
      item = Repo.get!(PhonesCollectionWeb.Item, conn.params["item_id"])
