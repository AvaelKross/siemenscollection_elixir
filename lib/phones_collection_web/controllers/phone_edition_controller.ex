defmodule PhonesCollectionWeb.PhoneEditionController do
  use SiemensCollection.Web, :controller

  alias PhonesCollectionWeb.{Phone, PhoneEdition}

  plug :scrub_params, "phone_edition" when action in [:create, :update]

  plug PhonesCollectionWeb.Plugs.CheckAdminRights when action in [:new, :create, :edit, :update, :delete]
    ...
    case Repo.insert(changeset) do
      {:ok, phone_edition} ->
        redirect_path = if conn.params["save_and_upload"] != nil do
          Routes.catalog_picture_path(conn, :new, conn.assigns.brand.id, conn.assigns.phone.id, phone_edition.id)
        else
          Routes.catalog_phone_edition_path(conn, :show, conn.assigns.brand.id, conn.assigns.phone.id, phone_edition.id)
        end
        conn
  ...
  def update(conn, %{"id" => id, "phone_edition" => phone_edition_params}) do
    ...
    case Repo.update(changeset) do
      {:ok, _phone_edition} ->
        ...
        |> redirect(to: Routes.catalog_phone_edition_path(conn, :show, conn.assigns.brand.id, conn.assigns.phone.id, phone_edition.id))
      {:error, changeset} ->
        render(conn, "edit.html", phones: phones, phone_edition: phone_edition, changeset: changeset)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    ...
    if conn.assigns.phone.main_edition_id == phone_edition.id do
      ...
      |> redirect(to: Routes.catalog_phone_path(conn, :show, conn.assigns.brand.id, conn.assigns.phone.id))
    else
