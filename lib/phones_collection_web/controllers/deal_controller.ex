defmodule PhonesCollectionWeb.DealController do
  use SiemensCollection.Web, :controller

  alias PhonesCollectionWeb.{Deal, PhoneEdition, User}
  ...
  def create(conn, %{"deal" => deal_params}) do
    ...
    case Repo.insert(changeset) do
      {:ok, deal} ->
        ...
        |> redirect(to: Routes.deal_path(conn, :show, deal.id))
      {:error, changeset} ->
        phone_editions = preload_editions
        render(conn, "new.html", phone_editions: phone_editions, changeset: changeset)
    end
  ...
  def update(conn, %{"deal" => deal_params}) do
    ...
    case Repo.update(changeset) do
      {:ok, deal} ->
        ...
        |> redirect(to: Routes.deal_path(conn, :show, deal))
      {:error, changeset} ->
        phone_editions = preload_editions
        render(conn, "edit.html", deal: deal, phone_editions: phone_editions, changeset: changeset)
    end
  ...
  def delete(conn, _params) do
    ...
    |> redirect(to: Routes.deal_path(conn, :index))
  end
