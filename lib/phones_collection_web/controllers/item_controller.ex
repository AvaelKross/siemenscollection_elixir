defmodule PhonesCollectionWeb.ItemController do
  use SiemensCollection.Web, :controller

  alias PhonesCollectionWeb.{Item, PhoneEdition, User}
  ...
  def new(conn, _params) do
    hash = cond do
      conn.params["edition_id"] ->
        %Item{user_id: Addict.Helper.current_user(conn).id, phone_edition_id: conn.params["edition_id"]}
      conn.params["deal_id"] ->
        deal = Repo.get!(PhonesCollectionWeb.Deal, conn.params["deal_id"])
        %Item{user_id: Addict.Helper.current_user(conn).id, deal_id: deal.id, phone_edition_id: deal.phone_edition_id}
      true ->
        %Item{user_id: Addict.Helper.current_user(conn).id}
    end
    ...
    case Repo.insert(changeset) do
      {:ok, item} ->
        redirect_path = if conn.params["save_and_upload"] != nil do
          Routes.collections_picture_path(conn, :new, conn.assigns.user.id, item.id)
        else
          Routes.collections_item_path(conn, :show, conn.assigns.user.id, item.id)
        end
        conn
  ...
  def update(conn, %{"item" => item_params}) do
    ...
    case Repo.update(changeset) do
      {:ok, item} ->
        ...
        |> redirect(to: Routes.collections_item_path(conn, :show, conn.assigns.user.id, item))
      {:error, changeset} ->
        phone_editions = preload_editions
        render(conn, "edit.html", item: item, phone_editions: phone_editions, changeset: changeset)
    end
  ...
  def delete(conn, _params) do
    ...
    |> redirect(to: Routes.ollections_item_path(conn, :index, conn.assigns.user.id))
  end
  ...
  defp preload_editions do
    ...
    |> Enum.sort_by(&("#{PhonesCollectionWeb.PhoneEditionView.full_phone_name(&1)}"))
  end
